export function renderErrorPage(err?: unknown): string {
  const details = err ? (err instanceof Error ? `${err.name}: ${err.message}\n${err.stack}` : String(err)) : "";
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    ...
    <style>
      ...
      pre { text-align: left; background: #eee; padding: 1rem; border-radius: 0.5rem; font-size: 12px; overflow: auto; max-height: 200px; }
