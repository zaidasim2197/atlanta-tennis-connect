...
| Concurrent real Stripe calls           | On 22 September 2026, 12 simultaneous staging registrations targeted the 3-place `l-hot` league with `PAYMENT_PROVIDER=stripe`. Exactly 3 returned 201 and 9 returned 409. Stripe API retrieval confirmed 3 distinct test-mode PaymentIntents whose metadata matched the 3 reservation IDs. No unexpected response or payment-creation failure occurred. Cleanup cancelled all 3 intents and restored all 3 places. See `docs/evidence/2026-09-22-stripe-concurrency.md`. | Pass   |
