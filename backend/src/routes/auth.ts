...
const signup = credentials.extend({ password: z.string().min(15).max(128), firstName: z.string().trim().min(1).max(80), lastName: z.string().trim().min(1).max(80), phone: z.string().max(40).optional(), city: z.string().max(100).optional(), ntrp: z.enum(["2.5", "3.0", "3.5", "4.0", "4.5+"]) }).strict();
